...
class DataFrameProcessor:
    ...
    def __init__(self, df: pd.DataFrame | None = None):
        ...
        self.df = df.copy()
        logger.info(
            "Step 1 - Initialized DataFrameProcessor with a DataFrame."  # pragma: no mutate
        )
    ...
    def rename_columns(self, columns_dict: dict[str, str]) -> None:
        ...
        if missing_columns:
            raise KeyError(
                f"The following columns are missing in the DataFrame: {', '.join(missing_columns)}"  # pragma: no mutate
            )
    ...
    def _get_column_formatter(self) -> ColumnFormatter:
        current_df = self.df
        instance_attr = "_cached_column_formatter"  # pragma: no mutate
        df_attr = "_cached_column_formatter_df"  # pragma: no mutate
        if (
            getattr(self, instance_attr, None) is None
            or getattr(self, df_attr, None) is not current_df
        ):
